...
const VALID_RECIPIENT = "GBYZFLD7ZQTSSDLWEMSD6UML2MTU4KKNCH765GZOVHAYKZNRJMWV4GMF";
...
function mockNoNonceSupport(client: SoroStreamClient) {
  vi.spyOn(client as any, "simulateOp").mockImplementation(
    (op: unknown) => {
      ...
      return Promise.resolve({ error: "missing entry", ...{} } as unknown as rpc.Api.SimulateTransactionErrorResponse);
    }
  );
}
...
function mockNonceSupport(client: SoroStreamClient) {
  vi.spyOn(client as any, "simulateOp").mockImplementation(() =>
    Promise.resolve({
      result: { retval: nativeToScVal({ major: 2, minor: 0, nonce_support: true }, { type: "map" }) },
      latestLedger: 100,
    ...
    vi.spyOn(client as any, "simulateOp").mockResolvedValue({
      result: {
        retval: nativeToScVal(
          { major: 2, minor: 0, nonce_support: true },
          { type: "map" }
        ),
      },
      latestLedger: 100,
