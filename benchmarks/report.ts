...
async function measurePercentiles(
  fn: () => Promise<unknown>,
  n: number
): Promise<Percentiles> {
  const times: number[] = [];
  ...
  const p = (pct: number): number =>
    times[Math.ceil((pct / 100) * times.length) - 1] ?? 0;
...
async function runBenchmarks(): Promise<Record<string, Percentiles>> {
  ...
  results["getStream"] = await measurePercentiles(
    () => mock.getStream(seedId),
    ITERATIONS
  );
  process.stdout.write(`  getStream      P50=${results["getStream"]!.p50.toFixed(3)}ms\n`);

  results["getClaimable"] = await measurePercentiles(
    () => mock.getClaimable(seedId),
    ITERATIONS
  );
...
async function main(): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`Could not read baseline at ${baselinePath}. Run with --update-baseline first.\n`);
    process.exit(1);
  }
