...
import { calculateVestingSchedule, serializeStreamToJSON, deserializeStreamFromJSON, bigintReplacer, bigintReviver } from "../src/utils.js";
...
describe("Full vesting cliff-to-release lifecycle", () => {
  ...
  it("create stream with cliff 50%, verify claimable via vesting schedule at cliff boundary", async () => {
    ...
    const beforeCliff = calculateVestingSchedule(stream, cliffSeconds, startTime + cliffSeconds - 1);
