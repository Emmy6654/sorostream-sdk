...
describe("KmsWalletAdapter (issue #306, #309)", () => {
  ...
  it("rejects transaction submission if signing function rejects its promise", async () => {
    ...
    await expect(adapter.signTransaction(tx.toXDR(), "testnet")).rejects.toThrow("KMS Access Denied");
  });

  it("throws error if constructed with invalid options", () => {
    expect(() => createKmsWalletAdapter({ publicKey: "", sign: async () => new Uint8Array() })).toThrow(
      "KmsWalletAdapter requires a valid publicKey"
    );
