...
describe("#204 filterStreams", () => {
  const streams: Stream[] = [
    makeStream({ id: "1", sender: SENDER_A, recipient: RECIPIENT_A, token: TOKEN_A, status: "Active" }),
    makeStream({ id: "2", sender: SENDER_B, recipient: RECIPIENT_A, token: TOKEN_A, status: "Cancelled" }),
    makeStream({ id: "3", sender: SENDER_A, recipient: RECIPIENT_B, token: TOKEN_B, status: "Completed" }),
    makeStream({ id: "4", sender: SENDER_A, recipient: RECIPIENT_A, token: TOKEN_B, status: "Paused" }),
    makeStream({
      id: "5",
      sender: SENDER_A,
      recipient: RECIPIENT_A,
  ...
  it("returns an empty array when nothing matches", () => {
    expect(filterStreams(streams, { sender: "GNOBODY0000000000000000000000000000000000000000000000" })).toEqual([]);
  });
